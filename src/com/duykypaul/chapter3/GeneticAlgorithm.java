package com.duykypaul.chapter3;
...
public class GeneticAlgorithm {
	...
	public Population crossoverPopulation_2Point(Population population) {
		// Create new population
		Population newPopulation = new Population(population.size());
		...
		for (int populationIndex = 0; populationIndex < population.size(); populationIndex++) {
			...
			if (this.crossoverRate > Math.random() && populationIndex >= this.elitismCount) {
				// Initialize offspring
				Individual offspringPoint1 = new Individual(parent1.getChromosomeLength());
				Individual offspringPoint2 = new Individual(parent1.getChromosomeLength());

				// Find second parent
				Individual parent2 = this.selectParent(population);

				// Get random swap point
				int swapPoint1 = (int) (Math.random() * (parent1.getChromosomeLength() + 1));
				int swapPoint2 = (int) (Math.random() * (parent1.getChromosomeLength() - swapPoint1 + 1)) + swapPoint1;

				// Loop over genome
				for (int geneIndex = 0; geneIndex < parent1.getChromosomeLength(); geneIndex++) {
					// Use half of parent1's genes and half of parent2's genes
					if (geneIndex < swapPoint1) {
						offspringPoint1.setGene(geneIndex, parent1.getGene(geneIndex));
						offspringPoint2.setGene(geneIndex, parent2.getGene(geneIndex));
					} else if (geneIndex < swapPoint2) {
						offspringPoint1.setGene(geneIndex, parent2.getGene(geneIndex));
						offspringPoint2.setGene(geneIndex, parent1.getGene(geneIndex));
					} else {
						offspringPoint1.setGene(geneIndex, parent1.getGene(geneIndex));
						offspringPoint2.setGene(geneIndex, parent2.getGene(geneIndex));
					}
				...
				// Add offspring to new population
				newPopulation.setIndividual(populationIndex, offspringPoint1.getFitness() > offspringPoint2.getFitness() ? offspringPoint1 : offspringPoint2);
